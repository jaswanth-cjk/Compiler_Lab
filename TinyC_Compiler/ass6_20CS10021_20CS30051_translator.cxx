...
SymTable *curTable, *globalTable;  // Symbol Tables
...
ActivationRecord::ActivationRecord() : totOffset(0), offset(map<string, int>()) {}
...
Sym *SymTable::lookup(string name)
{
    ...
    {
        ret_ptr = new Sym(name);
        ret_ptr->category = Sym::LOCAL;
        if(curTable == globalTable)
            ret_ptr->category = Sym::GLOBAL;
...
void SymTable::update()
{
    ...
    for (auto map_entry : (this)->syms)	
    {	
        if (map_entry.second.category == Sym::PARAMETER)	
        {	
            if(map_entry.second.size != 0)	
            {	
                activationRecord->totOffset -= map_entry.second.size;	
                activationRecord->offset[map_entry.second.name] = activationRecord->totOffset;	
            }	
        }	
    }	
    for (auto map_entry : (this)->syms)	
    {	
        if ((map_entry.second.category == Sym::LOCAL && map_entry.second.name != "return") || map_entry.second.category == Sym::TEMPORARY)	
        {	
            if(map_entry.second.size != 0)	
            {	
                activationRecord->totOffset -= map_entry.second.size;	
...
void SymTable::print()
{
    ...
    for (auto &map_entry : (this)->syms)
    {
        ...
        cout << setw(20);	
        if(map_entry.second.category == Sym::LOCAL)
            cout << "local";	
        else if(map_entry.second.category == Sym::GLOBAL)
            cout << "global";	
        else if(map_entry.second.category == Sym::FUNCTION)
            cout << "function";	
        else if(map_entry.second.category == Sym::PARAMETER)
            cout << "parameter";	
        else if(map_entry.second.category == Sym::TEMPORARY)
            cout << "temporary";
...
Sym::Sym(string name, SymType::typeEnum type, string init) : name(name), type(new SymType(type)), offset(0), nestedTable(NULL), initialValue(init)
{
...
void Quad::print()
{
    ...
    {	
        cout << "\t" << this->result << " = " << strLiterals[atoi(this->arg1.c_str())] << endl;	
    }
    else if (this->op == "~")
    {
        shift_print_("= ~");
...
void finalBackpatch()	
{
    int curPos = qArray.size();	
    int lastExit = -1;	
    for(auto it = qArray.rbegin(); it != qArray.rend(); it++) {	
        string op = (*it)->op;	
        if(op == "labelend") 	
        {	
            lastExit = curPos;	

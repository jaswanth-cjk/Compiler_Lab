...
compound_statement:
                    LCUR new_block new_scope block_item_list_opt RCUR
                    {
                        $$ = $4;
                        changeTable(curTable->parent);
                    }
                    ;
new_block: 
                {
                    string name = curTable->name + "_" + toString(tableCnt);
                    tableCnt++;
                    Sym *s = curTable->lookup(name);
                    s->nestedTable = new SymTable(name, curTable);
                    s->type = new SymType(SymType::Block);
                    curSym = s;
                } 
                ;

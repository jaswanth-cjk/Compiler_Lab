...
{
    int len = 0;
    while (str[len] != '\0') // run it till it finds a null character
        len++;
...
int readInt(int *n)
{
    ...
    while (buff[i] != ' ' && buff[i] != '\n' && buff[i] != '\t')
    {
        if (buff[i] < '0' || buff[i] > '9') // verifying the invalid character
            return ERR;
        int digit = buff[i] - '0';

        // checking for range
        if (!isNeg && 1L * num * 10 + digit > MAX_INT_POSITIVE)
            return ERR;
        else if (isNeg && 1L * num * 10 + digit > MAX_INT_NEGATIVE)
            return ERR;
    ...
    if (isNeg) // if the original number was negative,then multiply by -1
        num *= -1;
    *n = (int)num;
    return OK;
}
...
int readFlt(float *f)
{
    ...
    if (len <= 0)
        return ERR;
    if ((buff[0] != '+') && (buff[0] != '-') && (buff[0] != '.') && (buff[0] < '0' || buff[0] > '9')) // checking for the valid first character
        return ERR;
